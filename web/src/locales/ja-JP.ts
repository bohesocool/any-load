export default {
  ...
  keys: {
    ...
    protocolConversionTooltip: "有効にすると、クライアントは OpenAI Chat 形式（/v1/chat/completions）でリクエストを送信し、プロキシが上流のサポートする形式に変換します。無効時はそのまま透過します。",
    upstreamFormats: "上流形式",
    upstreamFormatsTooltip: "上流がサポートする1つ以上の形式。OpenAI Chat を含む場合は変換せず透過、それ以外はリスト先頭の形式に変換します。プロトコル変換有効時にのみ適用されます。",
    selectUpstreamFormats: "上流がサポートする形式を選択",
    protocolConversionRequiresFormat: "プロトコル変換を有効にする場合は上流形式を1つ以上選択してください",
    streamMode: "ストリームモード",
    streamModeTooltip: "上流のストリームフラグとクライアントのストリーム意図を切り離します（プロトコル変換がオンで、かつ入站形式が実際に変換される場合のみ適用。スマート透過では無効）。パススルー：上流はクライアントに一致。擬似非ストリーム：上流を強制ストリーム化し、クライアントが非ストリーム希望時は1つのレスポンスにバッファ。擬似ストリーム：上流を強制非ストリーム化し、クライアントがストリーム希望時はSSEに展開。",
    streamModePassthrough: "パススルー（変換なし）",
    streamModeFakeNonStream: "擬似非ストリーム（上流はストリーム、クライアントは単一レスポンス）",
    streamModeFakeStream: "擬似ストリーム（上流は非ストリーム、クライアントはSSE）",
    selectStreamMode: "ストリームモードを選択",
    fuzzyFailover: "ファジーフェイルオーバー",
    fuzzyFailoverTooltip: "有効にすると、400（リクエスト本体エラー）を除くすべての上流エラーコードでフェイルオーバー（別キーでリトライ）します。400はリトライせずそのままクライアントへ返します。リトライ枯渇後、400以外の最終エラーコードは固定メッセージ「此请求错误请稍后再试」でマスクし、上流の生エラー本文を隠します（元のステータスコードは保持）。無効時は「フェイルオーバーステータスコード」設定に従います。",
    fuzzyFailoverOn: "有効",
    fuzzyFailoverOff: "無効",
    fuzzyFailoverInfo: "有効：400以外はすべてフェイルオーバー、最終エラーは「此请求错误请稍后再试」にマスク。",
    fuzzyFailoverMessage: "マスク文案",
    fuzzyFailoverMessageTooltip: "リトライ枯渇後にクライアントへ返すマスク文案。空欄ならクライアントの Accept-Language に従い自動多言語化（zh/en/ja）。入力すれば固定文案を使用（クライアント言語を無視）。",
    fuzzyFailoverMessagePlaceholder: "空欄ならクライアント言語で「リクエストエラーです。後でもう一度お試しください。」を自動返却",
    fuzzyFailoverMessageDescription: "空欄 = クライアント言語で自動多言語、入力 = 固定文案。",
    sortOrder: "並び順",
    testModel: "テストモデル",
    testPath: "テストパス",
    hideKeys: "キーを非表示",
    ...
    paramOverridesTooltip:
      "APIリクエストパラメータをルールとして上書きします。各ルールにはモード（set/delete/copy/move/append/prepend）、パス（messages.0.role のようなネストJSONパス、-1 は配列の末尾）、任意の値、任意の条件（条件が一致した場合のみ適用、例：model の接頭辞 gpt-）を指定します。変換前のインバウンドリクエストボディに適用されます。",
    addParamOverride: "上書きルールを追加",
    selectOperationMode: "モード",
    operationModeSet: "設定 (set)",
    operationModeDelete: "削除 (delete)",
    operationModeCopy: "コピー (copy)",
    operationModeMove: "移動 (move)",
    operationModeAppend: "追加 (append)",
    operationModePrepend: "先頭追加 (prepend)",
    operationPath: "パス (例: temperature、messages.0.role)",
    operationFrom: "元パス",
    operationTo: "先パス",
    operationValue: "値 (JSON: 0.7、\"テキスト\"、[1,2]、true)",
    keepOriginTooltip: "元を保持: パスが存在しない場合のみ値を設定",
    conditions: "条件",
    logicAnd: "AND",
    logicOr: "OR",
    conditionModeFull: "等しい",
    conditionModePrefix: "接頭辞",
    conditionModeSuffix: "接尾辞",
    conditionModeContains: "含む",
    conditionModeGt: ">",
    conditionModeGte: ">=",
    conditionModeLt: "<",
    conditionModeLte: "<=",
    conditionPath: "パス (例: model、request_path、is_stream)",
    conditionValue: "値",
    invertTooltip: "反転: 条件が一致しない場合に通過",
    passMissingKeyTooltip: "欠落時通過: パスが存在しない場合に一致とみなす",
    addCondition: "条件を追加",
    // パラメーターオーバーライド編集ツールバー / モード / テンプレート
    paramOverrideMode: "モード",
    paramOverrideVisual: "ビジュアル",
    paramOverrideJson: "JSON テキスト",
    paramOverrideTemplate: "テンプレート",
    paramOverrideFillTemplate: "テンプレートを適用",
    paramOverrideAppendTemplate: "テンプレートを追加",
    paramOverrideReset: "リセット",
    paramOverrideNoRules: "ルールがありません。「上書きルールを追加」またはテンプレートを読み込んでください。",
    paramOverrideAlwaysExecutes: "条件が未設定の場合、操作は常に実行されます。",
    paramOverrideLegacyHint: "旧形式フラット形式: トップレベル key → value の JSON オブジェクト（例: {\"temperature\": 0.7}）。",
    paramOverrideJsonHint: "JSON を直接編集。保存時にフォーマットを検証します。",
    paramOverrideInvalidJson: "JSON 形式エラー。ビジュアルに切り替える前に修正してください。",
    paramOverrideMustBeObject: "パラメーターオーバーライドは JSON オブジェクト（operations 配列、またはフラット key-value）である必要があります。",
    paramOverrideJsonFormatError: "JSON 形式エラー",
    paramOverrideTemplate_operations_default: "新形式テンプレート",
    paramOverrideTemplate_legacy_default: "旧形式テンプレート",
    paramOverrideTemplate_delete_field: "フィールド削除",
    paramOverrideTemplate_copy_field: "フィールドコピー",
    operationDescription: "ルールの説明（任意）",
    keepOrigin: "元を保持",
    invert: "反転",
    passMissingKey: "欠落時通過",
