export default {
  ...
  keys: {
    ...
    protocolConversionTooltip: "开启后，客户端统一使用 OpenAI Chat 格式（/v1/chat/completions）发送请求，代理将其转换到上游支持的格式。关闭则纯透传，行为不变。",
    upstreamFormats: "上游格式",
    upstreamFormatsTooltip: "上游支持的一种或多种格式。若包含 OpenAI Chat 则直接透传不转换；否则转换到列表中首个格式。仅开启协议转换时生效。",
    selectUpstreamFormats: "选择上游支持的格式",
    protocolConversionRequiresFormat: "开启协议转换时必须至少选择一个上游格式",
    streamMode: "流式模式",
    streamModeTooltip: "将上游的流式开关与客户端的流式意图解耦（仅在开启协议转换、且入站格式确实被转换时生效；智能透传下不生效）。直通：上游与客户端一致。假非流：强制上游流式，当客户端要非流时缓冲成一个完整响应。假流式：强制上游非流，当客户端要流时展开成 SSE。",
    streamModePassthrough: "直通（不转换）",
    streamModeFakeNonStream: "假非流（上游流式，客户端收到完整响应）",
    streamModeFakeStream: "假流式（上游非流，客户端收到 SSE）",
    selectStreamMode: "选择流式模式",
    fuzzyFailover: "模糊重定向",
    fuzzyFailoverTooltip: "开启后，除 400（请求体错误）外的所有上游错误码都触发故障转移（换 key 重试）；400 原样返回客户端不重试。重试耗尽后，任何非 400 的最终错误码向客户端返回固定文案「此请求错误请稍后再试」，隐藏上游原始错误体（保留原状态码）。关闭则沿用「故障转移状态码」配置。",
    fuzzyFailoverOn: "已开启",
    fuzzyFailoverOff: "已关闭",
    fuzzyFailoverInfo: "已开启：除 400 外全部故障转移，最终错误掩码为「此请求错误请稍后再试」。",
    fuzzyFailoverMessage: "掩码文案",
    fuzzyFailoverMessageTooltip: "重试耗尽后返回给客户端的掩码文案。留空则按客户端 Accept-Language 自动多语言（zh/en/ja）；填写则固定使用该文案（忽略客户端语言）。",
    fuzzyFailoverMessagePlaceholder: "留空则按客户端语言自动返回「此请求错误请稍后再试」",
    fuzzyFailoverMessageDescription: "留空 = 按客户端语言自动多语言；填写 = 固定文案。",
    sortOrder: "排序",
    testModel: "测试模型",
    testPath: "测试路径",
    hideKeys: "隐藏密钥",
    ...
    paramOverridesTooltip:
      "以规则形式覆盖 API 请求参数。每条规则含一个模式（set/delete/copy/move/append/prepend）、路径（嵌套 JSON 路径如 messages.0.role，-1 表示数组末尾）、可选值，以及可选条件（仅在条件匹配时应用，如 model 前缀 gpt-）。作用于转换前的入站请求体。",
    addParamOverride: "添加覆盖规则",
    selectOperationMode: "模式",
    operationModeSet: "设置 (set)",
    operationModeDelete: "删除 (delete)",
    operationModeCopy: "复制 (copy)",
    operationModeMove: "移动 (move)",
    operationModeAppend: "追加 (append)",
    operationModePrepend: "前置 (prepend)",
    operationPath: "路径 (如 temperature、messages.0.role)",
    operationFrom: "源路径",
    operationTo: "目标路径",
    operationValue: "值 (JSON: 0.7、\"文本\"、[1,2]、true)",
    keepOriginTooltip: "保留原值：仅当路径不存在时才设置",
    conditions: "条件",
    logicAnd: "AND",
    logicOr: "OR",
    conditionModeFull: "等于",
    conditionModePrefix: "前缀",
    conditionModeSuffix: "后缀",
    conditionModeContains: "包含",
    conditionModeGt: ">",
    conditionModeGte: ">=",
    conditionModeLt: "<",
    conditionModeLte: "<=",
    conditionPath: "路径 (如 model、request_path、is_stream)",
    conditionValue: "值",
    invertTooltip: "取反：当条件不匹配时通过",
    passMissingKeyTooltip: "缺失即通过：路径不存在时视为匹配",
    addCondition: "添加条件",
    // 参数覆盖编辑器工具栏 / 模式 / 模板
    paramOverrideMode: "模式",
    paramOverrideVisual: "可视化",
    paramOverrideJson: "JSON 文本",
    paramOverrideTemplate: "模板",
    paramOverrideFillTemplate: "填入模板",
    paramOverrideAppendTemplate: "追加模板",
    paramOverrideReset: "重置",
    paramOverrideNoRules: "暂无规则。点击「添加覆盖规则」或载入模板。",
    paramOverrideAlwaysExecutes: "未设置条件时，该操作总是执行。",
    paramOverrideLegacyHint: "旧版扁平格式：顶层 key → value 的 JSON 对象（如 {\"temperature\": 0.7}）。",
    paramOverrideJsonHint: "直接编辑 JSON，保存时会校验格式。",
    paramOverrideInvalidJson: "JSON 格式错误，请先修复再切换到可视化。",
    paramOverrideMustBeObject: "参数覆盖必须是 JSON 对象（含 operations 数组，或扁平 key-value）。",
    paramOverrideJsonFormatError: "JSON 格式错误",
    paramOverrideTemplate_operations_default: "新格式模板",
    paramOverrideTemplate_legacy_default: "旧格式模板",
    paramOverrideTemplate_delete_field: "删除字段",
    paramOverrideTemplate_copy_field: "复制字段",
    operationDescription: "规则描述（可选）",
    keepOrigin: "保留原值",
    invert: "取反",
    passMissingKey: "缺失即通过",
