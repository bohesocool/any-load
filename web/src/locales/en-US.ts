export default {
  ...
  keys: {
    ...
    protocolConversionTooltip: "When enabled, clients send OpenAI Chat format (/v1/chat/completions) and the proxy converts it to a format the upstream supports. When disabled, requests pass through unchanged.",
    upstreamFormats: "Upstream Formats",
    upstreamFormatsTooltip: "One or more formats the upstream supports. If OpenAI Chat is included, requests pass through without conversion; otherwise they are converted to the first format in the list. Only effective when protocol conversion is enabled.",
    selectUpstreamFormats: "Select formats the upstream supports",
    protocolConversionRequiresFormat: "At least one upstream format is required when protocol conversion is enabled",
    streamMode: "Stream Mode",
    streamModeTooltip: "Decouples the upstream stream flag from the client's stream intent (only when protocol conversion is on, and only when the inbound format is actually being converted — smart passthrough ignores it). Passthrough: upstream matches the client. Fake non-stream: force the upstream to stream and buffer it into one response when the client wanted non-stream. Fake stream: force the upstream to non-stream and expand it into SSE when the client wanted stream.",
    streamModePassthrough: "Passthrough (no conversion)",
    streamModeFakeNonStream: "Fake non-stream (upstream streams, client gets one response)",
    streamModeFakeStream: "Fake stream (upstream non-stream, client gets SSE)",
    selectStreamMode: "Select stream mode",
    fuzzyFailover: "Fuzzy Failover",
    fuzzyFailoverTooltip: "When enabled, every upstream error code except 400 (bad request body) triggers failover (retry on another key); 400 is returned to the client directly without retry. After retries are exhausted, any non-400 final error is masked with a fixed message \"此请求错误请稍后再试\" (this request failed, please try again later), hiding the raw upstream error body (the original status code is kept). When disabled, the Failover Status Codes config applies as usual.",
    fuzzyFailoverOn: "Enabled",
    fuzzyFailoverOff: "Disabled",
    fuzzyFailoverInfo: "Enabled: all non-400 errors failover; final errors are masked as \"此请求错误请稍后再试\".",
    fuzzyFailoverMessage: "Mask Message",
    fuzzyFailoverMessageTooltip: "The masked message returned to the client after retries are exhausted. Leave empty to auto-localize per the client's Accept-Language (zh/en/ja); fill in to use a fixed message (ignoring the client language).",
    fuzzyFailoverMessagePlaceholder: "Leave empty to auto-localize as \"This request failed, please try again later.\"",
    fuzzyFailoverMessageDescription: "Empty = auto-localize by client language; filled = fixed message.",
    ...
    paramOverridesTooltip:
      "Override API request parameters as rules. Each rule has a mode (set/delete/copy/move/append/prepend), a path (nested JSON path like messages.0.role, -1 for last array item), an optional value, and optional conditions (only apply when conditions match, e.g. model prefix gpt-). Applied to the inbound request body before conversion.",
    addParamOverride: "Add Override Rule",
    selectOperationMode: "Mode",
    operationModeSet: "Set",
    operationModeDelete: "Delete",
    operationModeCopy: "Copy",
    operationModeMove: "Move",
    operationModeAppend: "Append",
    operationModePrepend: "Prepend",
    operationPath: "Path (e.g. temperature, messages.0.role)",
    operationFrom: "From path",
    operationTo: "To path",
    operationValue: "Value (JSON: 0.7, \"text\", [1,2], true)",
    keepOriginTooltip: "Keep origin: only set the value if the path does not already exist",
    conditions: "Conditions",
    logicAnd: "AND",
    logicOr: "OR",
    conditionModeFull: "equals",
    conditionModePrefix: "prefix",
    conditionModeSuffix: "suffix",
    conditionModeContains: "contains",
    conditionModeGt: ">",
    conditionModeGte: ">=",
    conditionModeLt: "<",
    conditionModeLte: "<=",
    conditionPath: "Path (e.g. model, request_path, is_stream)",
    conditionValue: "Value",
    invertTooltip: "Invert: pass the condition when it does NOT match",
    passMissingKeyTooltip: "Pass on missing key: treat a missing path as matching",
    addCondition: "Add Condition",
    // param-override editor toolbar / modes / templates
    paramOverrideMode: "Mode",
    paramOverrideVisual: "Visual",
    paramOverrideJson: "JSON Text",
    paramOverrideTemplate: "Template",
    paramOverrideFillTemplate: "Fill Template",
    paramOverrideAppendTemplate: "Append Template",
    paramOverrideReset: "Reset",
    paramOverrideNoRules: "No rules. Click \"Add Override Rule\" or load a template.",
    paramOverrideAlwaysExecutes: "When no conditions are set, the operation always executes.",
    paramOverrideLegacyHint: "Legacy flat format: a JSON object of top-level key → value (e.g. {\"temperature\": 0.7}).",
    paramOverrideJsonHint: "Edit JSON directly. Format is validated on save.",
    paramOverrideInvalidJson: "Invalid JSON — fix it before switching to Visual.",
    paramOverrideMustBeObject: "Parameter override must be a JSON object (with an \"operations\" array, or a flat key-value map).",
    paramOverrideJsonFormatError: "JSON format error",
    paramOverrideTemplate_operations_default: "New Format Template",
    paramOverrideTemplate_legacy_default: "Legacy Format Template",
    paramOverrideTemplate_delete_field: "Delete Field",
    paramOverrideTemplate_copy_field: "Copy Field",
    operationDescription: "Rule description (optional)",
    keepOrigin: "Keep origin",
    invert: "Invert",
    passMissingKey: "Pass if missing",
  ...
  logs: {
    ...
    upstreamRequestContent: "Upstream Request Content",
    clientResponseContent: "Client Response Content",
